...
namespace platform
{
	static_assert(std::is_same<mode_t, leo::underlying_type_t<Mode>>::value,
	...
	namespace
	{
		...
		//@{
		LB_NONNULL(1) bool
			UnlinkWithAttr(const wchar_t* path, FileAttributes attr) lnothrow
		{
		...
		//@}
#else
		inline PDefH(::timespec, ToTimeSpec, FileTime ft) lnothrow
...
#	define UTIME_OMIT (-1L)
#endif
			lunused(fd), lunused(times);
			leo::throw_error(std::errc::function_not_supported, lfsig);
#else
			LCL_CallF_CAPI(, ::futimens, fd, times);
#endif
		}
		...
		inline PDefH(FileNodeID, get_file_node_id, struct ::stat& st) lnothrow
			ImplRet({ std::uint64_t(st.st_dev), std::uint64_t(st.st_ino) })
			//@}
			//! \since build 719
			inline LB_NONNULL(2, 4) PDefH(void, cstat, struct ::stat& st,
				const char16_t* path, bool follow_link, const char* sig)
			ImplRet(cstat(st, MakePathString(path).c_str(), follow_link, sig))
			//! \since build 632
			inline LB_NONNULL(2) PDefH(int, estat, struct ::stat& st, const char16_t* path,
				bool follow_link)
...
#else
			-> leo::result_of_t<_func(struct ::stat&)>
#endif
		{
#if LFL_Win32
				...
				_tParams&&... args)
		{
			while (nbyte > 0)
			{
				const auto n(leo::invoke(f, obj, ptr, nbyte, yforward(args)...));
		...
		size_t
			SafeReadWrite(_func f, int fd, _tBuf buf, rwsize_t nbyte) lnothrowv
		{
			LAssert(fd != -1, "Invalid file descriptor found.");
...
#	else
			...
			LAssert(res == 0, "Narrow contract violated.");
			lunused(res);
#	endif
		}
#endif
		...
		bool
			IsNodeShared_Impl(const char* a, const char* b, bool follow_link) lnothrow
		{
#if LFL_Win32
			return CallNothrow({}, [=] {
		...
		LB_NONNULL(1, 2) pair<UniqueFile, UniqueFile>
	...
	NodeCategory
		CategorizeNode(mode_t st_mode) lnothrow
	{
	...
	FileDescriptor::FileDescriptor(std::FILE* fp) lnothrow
		: desc(fp ? LCL_CallGlobal(fileno, fp) : -1)
	{}
	...
	NodeCategory
		FileDescriptor::GetCategory() const lnothrow
	{
#if LFL_Win32
	...
	{
#if LCL_API_POSIXFileSystem
		return LCL_CallF_CAPI(, ::fcntl, desc, F_GETFL);
#else
		leo::throw_error(std::errc::function_not_supported, lfsig);
#endif
	}
	...
	FileNodeID
		FileDescriptor::GetNodeID() const lnothrow
	{
#if LFL_Win32
		return CallNothrow({}, [=] {
	...
	size_t
		FileDescriptor::GetNumberOfLinks() const lnothrow
	{
#if LFL_Win32
		return CallNothrow({}, [=] {
...
#else
		const ::timespec times[]{ ToTimeSpec(ft),{ limpl(0), UTIME_OMIT } };
	...
	{
#if LCL_API_POSIXFileSystem
		LCL_CallF_CAPI(, ::fcntl, desc, F_SETFL, flags);
#else
	...
	{
#if LCL_API_POSIXFileSystem
		LCL_CallF_CAPI(, ::fchmod, desc, mode);
#else
		lunused(mode);
		leo::throw_error(std::errc::function_not_supported, lfsig);
#endif
...
#else
		const ::timespec times[]{ { limpl(0), UTIME_OMIT }, ToTimeSpec(ft) };
	...
	bool
		FileDescriptor::SetSize(size_t size) lnothrow
	{
#if LFL_Win32
	...
	int
		FileDescriptor::SetTranslationMode(int mode) const lnothrow
	{
		// TODO: Other platforms.
#if LFL_Win32 || (defined(_NEWLIB_VERSION) && defined(__SCLE))
		return LCL_CallGlobal(setmode, desc, mode);
#else
		// NOTE: No effect.
		lunused(mode);
		return 0;
	...
	{
#if LFL_Win32
		LCL_CallF_CAPI(, ::_commit, desc);
#else
		// XXX: http://austingroupbugs.net/view.php?id=672.
		LCL_CallF_CAPI(, ::fsync, desc);
#endif
	}

	size_t
		FileDescriptor::FullRead(void* buf, size_t nbyte) lnothrowv
	{
	...
	size_t
		FileDescriptor::FullWrite(const void* buf, size_t nbyte) lnothrowv
	{
		using namespace std::placeholders;
	...
	size_t
		FileDescriptor::Read(void* buf, size_t nbyte) lnothrowv
	{
		return SafeReadWrite(LCL_ReservedGlobal(read), desc, buf, rwsize_t(nbyte));
	}

	size_t
		FileDescriptor::Write(const void* buf, size_t nbyte) lnothrowv
	{
		return SafeReadWrite(LCL_ReservedGlobal(write), desc, buf, rwsize_t(nbyte));
	}
			...
			byte* buf, size_t size)
	{
		LAssertNonnull(ifd),
			LAssertNonnull(ofd),
			LAssertNonnull(buf),
			LAssert(size != 0, "Invalid size found.");

		leo::retry_on_cond([&](size_t len) {
			if (len == size_t(-1))
				LCL_Raise_SysE(, "Failed reading source file '" + to_string(*ifd),
					lfsig);
			if (ofd.FullWrite(buf, len) == size_t(-1))
				LCL_Raise_SysE(,
					"Failed writing destination file '" + to_string(*ofd), lfsig);
			return len != 0;
	...
	void
		FileDescriptor::WriteContent(FileDescriptor ofd, FileDescriptor ifd,
			size_t size)
	{
		leo::temporary_buffer<byte> buf(size);
...
#else
		LCL_CallF_CAPI(, ::flock, desc, LOCK_EX);
#endif
	}
...
#else
		LCL_CallF_CAPI(, ::flock, desc, LOCK_SH);
#endif
	}

	bool
		FileDescriptor::try_lock() lnothrowv
	{
#if YCL_DS
		return true;
#elif LFL_Win32
	...
	bool
		FileDescriptor::try_lock_shared() lnothrowv
	{
#if YCL_DS
		return true;
#elif LFL_Win32
	...
	void
		FileDescriptor::unlock() lnothrowv
	{
#if YCL_DS
#else
		UnlockFileDescriptor(desc, lfsig);
#endif
	}

	void
		FileDescriptor::unlock_shared() lnothrowv
	{
#if YCL_DS
#else
		UnlockFileDescriptor(desc, lfsig);
	...
	void
		SetBinaryIO(std::FILE* stream) lnothrowv
	{
#if LFL_Win32
	...
	int
		omode_conv(std::ios_base::openmode mode) lnothrow
	{
		using namespace std;
	...
	int
		omode_convb(std::ios_base::openmode mode) lnothrow
	{
	...
	int
		uopen(const char* filename, int oflag, mode_t pmode) lnothrowv
	{
	...
	int
		uopen(const char16_t* filename, int oflag, mode_t pmode) lnothrowv
	{
		LAssertNonnull(filename);
#if LFL_Win32
	...
	{
		if (const auto c_mode = leo::openmode_conv(mode))
	...
	bool
		ufexists(const char* filename) lnothrowv
	{
#if LFL_Win32
		return leo::call_value_or(leo::compose(std::fclose,
			leo::addrof<>()), ufopen(filename, "rb"), limpl(1)) == 0;
#else
		return leo::fexists(filename);
#endif
	}
	bool
		ufexists(const char16_t* filename) lnothrowv
	{
		return leo::call_value_or(leo::compose(std::fclose,
			leo::addrof<>()), ufopen(filename, u"rb"), limpl(1)) == 0;
	}
			...
			try
			{
				...
				if (const auto cwd = ::_wgetcwd(p.get(), int(size)))
				{
					...
					if (size < len + 1)
						...
						return leo::ntctscpy(buf, res.data(), len);
				}
			}
	...
	char16_t*
		ugetcwd(char16_t* buf, size_t len) lnothrowv
	{
		LAssertNonnull(buf);
		if (len != 0)
		{
			...
			if (const auto cwd
				= ::getcwd(leo::aligned_store_cast<char*>(buf), len))
				try
			{
				...
				if (len < rlen + 1)
					...
					return leo::ntctscpy(buf, res.data(), rlen);
			}
			CatchExpr(std::bad_alloc&, errno = ENOMEM)
#endif
		}
...
FetchCurrentWorkingDirectory(size_t)
{
	...
	leo::retry_on_cond([&]() -> bool {
		...
		if (rlen != 0)
			return true;
		LCL_Raise_Win32E("GetCurrentDirectoryW", lfsig);
	}, [&] {
...
FetchNumberOfLinks(const char* path, bool follow_link)
{
	...
	cstat(st, path, follow_link, lfsig);
	return st.st_nlink;
#endif
}
LB_NONNULL(1) size_t
...
EnsureUniqueFile(const char* dst, mode_t mode, size_t allowed_links,
	bool share)
{
	static lconstexpr const auto oflag(OpenMode::Create
	...
	const char* name_b)
{
		...
		|| bool(Nonnull(p_b)->GetCategory() & NodeCategory::Directory)))
	{
		...
		if (!fb_a.open(std::move(p_a),
			...
			leo::throw_error(std::errc::bad_file_descriptor,
		...
		return leo::streambuf_equal(fb_a, fb_b);
	}
	return {};
}

bool
IsNodeShared(const char* a, const char* b, bool follow_link) lnothrowv
{
	return a == b || leo::ntctscmp(Nonnull(a), Nonnull(b)) == 0
		|| IsNodeShared_Impl(a, b, follow_link);
}
bool
IsNodeShared(const char16_t* a, const char16_t* b, bool follow_link) lnothrowv
{
	return a == b || leo::ntctscmp(Nonnull(a), Nonnull(b)) == 0
		|| IsNodeShared_Impl(a, b, follow_link);
}
bool
IsNodeShared(FileDescriptor x, FileDescriptor y) lnothrow
{
