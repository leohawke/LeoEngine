...
sexp_t *pack(char *tag, float v1, float v2, float v3) {
  sexp_t *sx = new_sexp_list(new_sexp_atom(tag,strlen(tag),atom_t::SEXP_BASIC));
  /* sx = (tag) */
  char sbuf[32];
  sexp_t *vlist,*vptr;
  ...
  sprintf(sbuf,"%f",v1);
  vlist->list = new_sexp_atom(sbuf,strlen(sbuf),atom_t::SEXP_BASIC); /* vlist = (v1) */
  vptr = vlist->list;

  /* sx = (tag (v1)) */

  sprintf(sbuf,"%f",v2);
  vptr->next = new_sexp_atom(sbuf, strlen(sbuf), atom_t::SEXP_BASIC); /* vlist = (v1 v2) */
  vptr = vptr->next;

  /* sx = (tag (v1 v2)) */

  sprintf(sbuf,"%f",v3);
  vptr->next = new_sexp_atom(sbuf, strlen(sbuf), atom_t::SEXP_BASIC); /* vlist = (v1 v2 v3) */
