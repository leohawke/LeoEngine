...
#define LS_Impl_LSLA1_Enable_TailRewriting true
#define LS_Impl_LSLA1_Enable_Thunked true
	...
	namespace v1
	{
		...
		namespace
		{
					...
					const ValueObject& delim, const string& name)
			{
				...
				TermNode res(AsNode(name, lforward(term).Value));

				if (IsBranch(term))
				{
					// NOTE: Explicit type 'TermNode' is intended.
					res += TermNode(AsIndexNode(res, pfx));
					leo::split(std::make_move_iterator(term.begin()),
						std::make_move_iterator(term.end()), leo::bind1(
			...
			{
				return TransformForSeparatorCore([&](_tTerm&& tm, const string&) lnothrow{
					return lforward(tm);
					}, lforward(term), pfx, delim, name);
			}
					...
					const ValueObject& delim, const TokenValue& name)
			{
				return TransformForSeparatorCore([&](_tTerm&& tm, const string& k) {
					return TransformForSeparatorRecursiveTmpl(lforward(tm), pfx, delim, k);
				}, lforward(term), pfx, delim, name);
			}
			//@}

			bool
				ExtractBool(TermNode& term, bool is_and) lnothrow
			{
				return leo::value_or(AccessTermPtr<bool>(term), is_and) == is_and;
			}
			...
			using EnvironmentGuard = leo::guard<EnvironmentSwitcher>;
...
#	if LS_Impl_LSLA1_Enable_TCO
			...
			public:
					...
					AttachFunction(ContextHandler&& h)
				{
					...
					//	added.
					xgds.emplace_back();
					leo::erase_all(xgds, h);
					// NOTE: Strong exception guarantee is kept here.
					swap(xgds.back(), h);
					return leo::as_const(xgds.back());
				}
					...
					MoveFunction()
				{
					...
					if (!xgds.empty())
					{
						const auto i(std::find_if(xgds.rbegin(), xgds.rend(),
							[this](const ContextHandler& h) lnothrow{
...
#if LS_Impl_LSLA1_Enable_Thunked
#	if LS_Impl_LSLA1_Enable_TailRewriting
			ReductionStatus
				ReduceSequenceOrderedAsync(TermNode& term, ContextNode& ctx, TNIter i)
			{
				LAssert(i != term.end(), "Invalid iterator found for sequence reduction.");
...
#	endif
				...
				EqualTerm(TermNode& term, _fComp f, _func g)
			{
				...
				term.Value = f(g(x), g(leo::as_const(Deref(++i))));
			}
			...
			private:
					...
					else if (const auto p = AccessPtr<TokenValue>(t))
					{
						...
						Forms::CheckParameterLeafToken(n, [&] {
							...
							env.GetMapRef()[n].SetContent(TermNode::Container(),
								ValueObject(leo::any_ops::use_holder, leo::in_place<
					...
					Restore(Environment& env, const TermNode& t)
				{
					if (IsBranch(t))
						...
						if (v.type() == leo::type_id<ContextHandler>())
						{
							// XXX: The element should exist.
							auto& p_strong(store.at(n));

							Deref(p_strong) = std::move(v.GetObject<ContextHandler>());
							v = ValueObject(leo::any_ops::use_holder,
								leo::in_place<HolderFromPointer<shared_ptr_t>>,
								std::move(p_strong));
						}
					});
				}
				...
				LB_NORETURN static ReductionStatus
			...
			{
				if (s != "#ignore" && !IsLSLASymbol(s))
					throw InvalidSyntax(leo::sfmt("Token '%s' is not a symbol or"
						" '#ignore' expected for %s.", s.data(), target));
			}

			LB_NORETURN LB_NONNULL(2) void
				ThrowInvalidSymbolType(const TermNode& term, const char* n)
			{
				throw InvalidSyntax(leo::sfmt("Invalid %s type '%s' found.", n,
						...
						if (p_saved)
						{
							...
							while (i != record_list.end())
							{
								...
								// TODO: Compress multiple inactive frames in one turn?
									...
									LAssert(n > 0, "Invalid weak count of frame found.");
									--n;
...
#elif LS_Impl_LSLA1_Enable_Thunked
			...
			class VauHandler final
				:private leo::equality_comparable<VauHandler>
			{
			private:
				/*!
				\brief 动态上下文名称。
				...
				*/
				VauHandler(string&& ename, shared_ptr<TermNode>&& p_fm,
					...
					owning_static(owning), p_static(owning ? std::move(p_env)
						: p_env->Anchor()), p_closure(share_move(leo::exchange(term,
					...
					operator==(const VauHandler& x, const VauHandler& y)
				{
					return x.eformal == y.eformal && x.p_formals == y.p_formals
						&& leo::ref_eq<>()(x.local_prototype, y.local_prototype)
			...
			{
				return leo::quote(std::string(TermToString(ReferenceTerm(term))), '\'');
			}
...
#	if LS_Impl_LSLA1_Enable_TCO
				const auto update_fused_act([&](TCOAction& fused_act) {
					...
					lf = {};
					lunseq(0, (lf = make_observer(&fused_act.AttachFunction(
...
#	endif
...
#else
			std::for_each(first, last, leo::bind1(ReduceCheckedSync, std::ref(ctx)));
#endif
		}
			...
			ReduceOnce(TermNode& term, ContextNode& ctx)
		{
			...
			return tp != leo::type_id<void>() && tp != leo::type_id<ValueToken>()
...
#	endif
				...
				throw ListReductionFailure(leo::sfmt("No matching combiner %s for"
		...
		{
			const auto res(leo::call_value_or([&](string_view id) -> ReductionStatus {
		...
		{
			bool
				IsSymbol(const string& id) lnothrow
			{
				return IsLSLASymbol(id);
			}
			...
			const string&
				SymbolToString(const TokenValue& s) lnothrow
			{
				return s;
			}
				...
				BindParameter(ContextNode& ctx, const TermNode& t, TermNode& o)
			{
				...
				MatchParameter(t, o, [&](TNIter first, TNIter last, string_view id) {
					LAssert(leo::begins_with(id, "."), "Invalid symbol found.");
					id.remove_prefix(1);
					if (!id.empty())
					{
					...
					std::function<void(const TokenValue&, TermNode&&)> bind_value)
			{
				if (IsBranch(t))
				{
					...
					if (n_p == n_o || (last != t.end() && n_o >= n_p - 1))
					{
						...
						if (last != t.end())
						{
							...
							LAssert(lastv.type() == leo::type_id<TokenValue>(),
				...
				else if (!t.Value)
				{
					...
					throw ParameterMismatch(leo::sfmt("Invalid parameter value found"
				...
				Undefine(TermNode& term, ContextNode& ctx, bool forced)
			{
				...
				if (term.size() == 2)
				{
					...
					if (IsLSLASymbol(n))
						...
						throw InvalidSyntax(leo::sfmt("Invalid token '%s' found as name"
				...
				Equal(TermNode& term)
			{
				EqualTermReference(term, [](const TermNode& x, const TermNode& y) {
					...
					return lx && ly ? leo::HoldSame(x.Value, y.Value) : &x == &y;
				});
			}
			...
			{
				EqualTermValue(term, leo::equal_to<>());
			}

			void
				EqualReference(TermNode& term)
			{
				EqualTermValue(term, leo::HoldSame);
			}
				...
				MakeEnvironment(TermNode& term)
			{
				...
				if (term.size() > 1)
				{
					...
					const auto tr([&](TNCIter iter) {
						return leo::make_transform(iter, [&](TNCIter i) {
				...
				WrapOnce(const ContextHandler& h)
			{
				...
				throw LSLException(leo::sfmt("Wrapping failed with type '%s'.",
					h.target_type().name()));
			}
				...
				Unwrap(const ContextHandler& h)
			{
				...
				throw LSLException(leo::sfmt("Unwrapping failed with type '%s'.",
					h.target_type().name()));
			}
