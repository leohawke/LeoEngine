float3 Sobel(float2 texcoord, Texture2D<float> tex, SamplerState s) {
	uint Width;
	uint Height;
	tex.GetDimensions(Width, Height);

	float2 dudv = float2(1.f / Width, 1.f / Height);
	float4 r = {
		tex.SampleLevel(s,texcoord + float2(-dudv.x,-dudv.y),0.f),
		tex.SampleLevel(s,texcoord + float2(0.f,-dudv.y),0.f),
		tex.SampleLevel(s,texcoord + float2(dudv.x,-dudv.y),0.f),
		tex.SampleLevel(s,texcoord + float2(-dudv.x,0.f),0.f)
	};
	float4 g =
	{
		tex.SampleLevel(s, texcoord + float2(dudv.x,0.f), 0.f),
		tex.SampleLevel(s, texcoord + float2(-dudv.x,dudv.y), 0.f),
		tex.SampleLevel(s, texcoord + float2(0.f, dudv.y), 0.f),
		tex.SampleLevel(s, texcoord + float2(dudv.x, dudv.y), 0.f)
	};

	//[ r.x r.y r.z ]   [ 1  2  1 ]
	//[ r.w  0  g.x ] * [ 0  0  0 ] = dx
	//[ g.y g.z g.w ]   [-1 -2 -1 ]
	//http://en.wikipedia.org/wiki/Sobel_operator
	float dx = r.x - r.z + 2.f*r.w - 2.f*g.x + g.y - g.w;
	//[ r.x r.y r.z ]   [ 1  0  -1 ]
	//[ r.w  0  g.x ] * [ 2  0  -2 ] = dy
	//[ g.y g.z g.w ]   [ 1 -0  -1 ]
	float dy = r.x + 2.f*r.y + r.z - g.y - 2.f*g.z - g.w;

	float dz = 0.01f*sqrt(max(0.f, 1.f - dx*dx - dy*dy));

	return float3(2.f*dx, dz, 2.f*dy);

}