...
float inoise(float2 p)
{
	...
	return 2 * (gNoiseTexure.SampleLevel(RepeatLinear, p, mipLevel).x - 0.5);	// [-1,1]
}
...
float inoise(float2 p)
{
	...
	const float mipLevel = 0;
	float4 n;
	n.x = gNoiseTexure.SampleLevel(SamplerRepeatPoint, i, mipLevel).x;
	n.y = gNoiseTexure.SampleLevel(SamplerRepeatPoint, i, mipLevel, int2(1, 0)).x;
	n.z = gNoiseTexure.SampleLevel(SamplerRepeatPoint, i, mipLevel, int2(0, 1)).x;
	n.w = gNoiseTexure.SampleLevel(SamplerRepeatPoint, i, mipLevel, int2(1, 1)).x;
