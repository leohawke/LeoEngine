#include "sampleNoise.hlsli"  // contains NLQs, NMQs, NHQs, etc.
...
float DENSITY(float3 ws)
{
	...
	float4 uulf_rand = saturate(NMQu(ws*0.000718, noiseVol0) * 2 - 0.5);
		...
		//-----------------------------------------------
		// PRE-WARP the world-space coordinate.包裹世界坐标
	...
	//-----------------------------------------------
	// compute 8 randomly-rotated versions of 'ws'.  旋转用来减少重复
	...
	float3 c0 = rot(ws, octaveMat0);
		...
		//-----------------------------------------------
		// MAIN SHAPE: CHOOSE ONE一个地板

#if 1
		// very general ground plane:
		density = -ws.y * 1;
	// to add a stricter ground plane further below:模拟沉淀
...
#endif  
#if 0
	// infinite network of caves:  (small bias)无尽三维洞窟
	density = 12;  // positive value -> more rock; negative value -> more open space给予更多岩石/空气
#endif
	...
	// CRUSTY SHELF
	// often creates smooth tops (~grass) and crumbly, eroded underneath parts.融合和下陷岩石
#if 1
	...
	// SPHERICAL TERRACES (for planet mode)球状地形
#if 0
	{
