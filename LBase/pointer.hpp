...
namespace leo
{
	...
	namespace details
	{
		...
		struct nptr_eq1_help {
			lconstexpr const static bool value = _type() == _type();
		};

		template<typename _type>
		struct nptr_eq2_help {
			lconstexpr const static bool value = _type(nullptr) == _type();
		};

		template<typename _type>
		using nptr_eq1 = bool_constant<nptr_eq1_help<_type>::value>;
		template<typename _type>
		using nptr_eq2 = bool_constant<nptr_eq1_help<_type>::value>;
