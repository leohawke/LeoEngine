...
#ifndef LBase_span_hpp
#define LBase_span_hpp 1
...
namespace absl {
	...
	public:
		...
		constexpr reference front() const noexcept {
			return lconstraint(size() > 0), * data();
		}
		...
		constexpr reference back() const noexcept {
			return lconstraint(size() > 0), * (data() + size() - 1);
		}
		...
		constexpr Span subspan(size_type pos = 0, size_type len = npos) const {
			return (pos <= size())
				...
				: (leo::throw_out_of_range("pos > size()"), Span());
		}
		...
		constexpr Span first(size_type len) const {
			return (len <= size())
				? Span(data(), len)
				: (leo::throw_out_of_range("len > size()"), Span());
		}
	...
	Span<T> MakeSpan(T* begin, T* end) noexcept {
		return lconstraint(begin <= end), Span<T>(begin, end - begin);
	}
	...
	Span<const T> MakeConstSpan(T* begin, T* end) noexcept {
		return lconstraint(begin <= end), Span<const T>(begin, end - begin);
	}
...
namespace leo
{
	...
	template <int&... ExplicitArgumentBarrier, typename C>
	constexpr auto make_const_span(const C& c) noexcept -> decltype(absl::MakeConstSpan(c)) {
		return absl::MakeConstSpan(c);
	}

	template <int&... ExplicitArgumentBarrier, typename T, size_t N>
	constexpr span<const T> make_const_span(const T(&array)[N]) noexcept {
		return absl::MakeConstSpan(array);
	}
}
