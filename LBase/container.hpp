...
#ifndef LBase_container_hpp
#define LBase_container_hpp 1

#include "LBase/iterator.hpp" // for begin, end, make_transform, size, std::distance,
//	std::make_move_iterator, cbegin, cend, std::piecewise_construct_t,
//	std::piecewise_construct;
#include "LBase/functional.hpp" // for std::declval, is_detected_convertible;
#include "LBase/utility.hpp" // for std::initializer_list, leo::as_const;
#include "LBase/algorithm.hpp" // for is_undereferenceable, sort_unique;
...
namespace leo
{
	...
	*/
	...
	public:
		...
		const container_type&
			get_container() const lnothrow
		{
			return *this;
		}

		void
			swap(container_adaptor& con) lnothrow
		{
	...
	*/
	...
	{
		return _tCon(begin(lforward(c)), end(lforward(c)));
	}
	template<class _tCon, typename _tRange, typename _func>
	inline _tCon
		make_container(_tRange&& c, _func f)
	{
		return _tCon(leo::make_transform(begin(c), f),
			leo::make_transform(end(c), f));
	}
	//@}
	...
	*/
	...
	{
		con.insert(end(con), begin(lforward(c)), end(lforward(c)));
	}
	//@}
	...
	*/
	...
	public:
		...
		auto
			...
			-> decltype(container->insert(lforward(args)...))
		{
			lassume(container);
	...
	*/
	...
	{
		leo::seq_apply(container_inserter<_tCon>(con), lforward(args)...);
	}
	...
	*/
	...
	\note 对整个序列容器使用 leo::remove 移除元素范围。
	...
	*/
	...
	\note 对整个序列容器使用 leo::remove_if 移除元素范围。
	...
	*/
	...
	{
		lassume(n <= std::distance(i, cend(con)));
	...
	typename _tCon::iterator
		erase_n(_tCon& con, typename _tCon::iterator i,
			typename _tCon::difference_type n)
	{
		lassume(n <= std::distance(i, end(con)));
		return con.erase(i, std::next(i, n));
	}
	//@}
	...
	*/
	...
	{
		return{ leo::cast_mutable(con, pr.first), pr.second };
	}
	//@}
	...
	*/
	...
	{
		con.erase(leo::sort_unique(begin(con), end(con)), end(con));
	}
	...
	*/
		...
		trail(_tCon&& con, _tParams&&... args)
	{
		if (!con.empty())
		{
			typename decay_t<_tCon>::value_type val(lforward(args)...);
	...
	*/
		...
		vector_concat(_tVector& vec, _tIn first, _tIn last)
	{
		...
		leo::concat(vec, first, last);
	}
	template<class _tVector, typename _tRange>
	void
		vector_concat(_tVector& vec, _tRange&& c)
	{
		vec.reserve(size(vec) + leo::range_size(c));
		leo::concat(vec, lforward(c));
	}
	//@}
	...
	*/
		...
		retry_for_vector(typename _tVector::size_type s, _func f)
	{
		...
		for (res.resize(s); f(res, s) && s < res.max_size() limpl(/ 2); )
			res.resize(s *= limpl(2));
		return res;
	}
	...
	*/
	...
	template<class _tAssocCon, typename _type,
		limpl(typename = enable_if_convertible_t<const _type&,
	...
	template<class _tAssocCon, typename _tKey,
		limpl(typename = enable_if_inconvertible_t<const _tKey&,
			const typename _tAssocCon::value_type&>)>
		inline const _tKey&
	...
	*/
	...
	{
		return leo::cast_mutable(con, leo::search_map(leo::as_const(con), k));
	}
	//@}
	//! \since build 1.4
	//@{
	...
	*/
			...
			const _tKey& k)
	{
		if (!con.empty())
		{
			...
			return leo::search_map(con, k);
		}
		lconstraint(hint == cend(con));
		return{ hint, true };
	}
	template<class _tAssocCon, typename _tKey>
	inline std::pair<typename _tAssocCon::iterator, bool>
			...
			const _tKey& k)
	{
		return leo::cast_mutable(con,
			leo::search_map(leo::as_const(con), hint, k));
	}
	//@}
	/*!
	...
	*/
	...
	{
		return details::insert_or_assign(leo::search_map(con, k), con,
			lforward(k), lforward(arg));
	}
	...
	inline std::pair<typename _tAssocCon::iterator, bool>
		insert_or_assign_hint(_tAssocCon& con, typename _tAssocCon::const_iterator hint,
			_tKey&& k, _tParam&& arg)
	{
		return details::insert_or_assign(leo::search_map(con, hint, k), con,
			lforward(k), lforward(arg));
	}
	//@}
	//@}
