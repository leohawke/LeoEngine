 ...
 public:
  ...
  ~hazptr_domain() {
    ...
    if (kIsDebug && !tagged_empty()) {
     ldebug("Tagged objects remain. This may indicate a higher-level leak ""of object(s) that use hazptr_obj_cohort.");
    }
  }
 ...
 private:
  ...
  void invoke_reclamation_in_executor(int rcount) {
    ...
    if (true) {
     ...
     recl_fn();
      ...
      if (kIsDebug) {
        ldebug("INFO Skip asynchronous reclamation by hazptr executor");
      }
    }
    if (backlog >= 10) {
  ...
  FOLLY_EXPORT FOLLY_NOINLINE void hazptr_warning_list_too_large(
    ...
    if ((warning_count++ % 10000) == 0) {
        ldebug("WARNINGHazptr retired list too large: tag = %u shard=%u count=%u", tag, shard, count);
    }
  }
  ...
  FOLLY_EXPORT FOLLY_NOINLINE void hazptr_warning_executor_backlog(
    ...
    static std::atomic<uint64_t> warning_count{0};
    if ((warning_count++ % 10000) == 0) {
      ldebug("WARNING %d %s",backlog, " request backlog for hazptr asynchronous "
