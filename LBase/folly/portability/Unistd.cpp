...
int pipe(int pth[2]) {
    ...
    auto r = netops::socketpair(PF_UNIX, SOCK_STREAM, 0, pair);
    if (r == -1) {
        return r;
    }

    auto network_socket_to_fd = [](NetworkSocket sock) {
        return netops::detail::SocketFileDescriptorMap::socketToFd(sock.data);
    };

    pth[0] = network_socket_to_fd(pair[0]);
    pth[1] = network_socket_to_fd(pair[1]);
    return 0;
}
...
//change:remove socket file support
