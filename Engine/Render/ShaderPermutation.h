...
namespace platform::Render
{
	inline namespace Shader
	{
		...
		struct FShaderPermutationBool
		{
			...
			{
				LAssert(PermutationId == 0 || PermutationId == 1, leo::sfmt("Invalid shader permutation dimension id %i.", PermutationId).c_str());
		...
		struct TShaderPermutationInt
		{
			...
			static int32 ToDimensionValueId(Type E)
			{
				...
				LAssert(PermutationId < PermutationCount&& PermutationId >= 0, leo::sfmt("Unknown shader permutation dimension value id %i.",PermutationId).c_str());
			...
			static Type FromDimensionValueId(int32 PermutationId)
			{
				LAssert(PermutationId < PermutationCount&& PermutationId >= 0, leo::sfmt("Invalid shader permutation dimension value id %i.", PermutationId).c_str());
		...
		struct TShaderPermutationSparseInt
		{
			...
			static int32 ToDimensionValueId(Type E)
			{
				LAssert(false, leo::sfmt("Unknown shader permutation dimension value %i.", E).c_str());
				return int32(0);
			}
			...
			static Type FromDimensionValueId(int32 PermutationId)
			{
				LAssert(false, leo::sfmt("Invalid shader permutation dimension id %i.", PermutationId).c_str());
				return Type(0);
			}
		};
		...
		*/
		...
		struct TShaderPermutationDomain
		{
			...
			{
				LAssert(PermutationId == 0, leo::sfmt("Invalid shader permutation id %i.", PermutationId).c_str());
			}
		...
		struct TShaderPermutationDomain<TDimension, Ts...>
		{
			...
			{
				LAssert(PermutationId >= 0 && PermutationId < PermutationCount, leo::sfmt("Invalid shader permutation id %i.", PermutationId).c_str());
			}
...
#define SHADER_PERMUTATION_SPARSE_INT(InDefineName,...) \
	DECLARE_SHADER_PERMUTATION_IMPL(InDefineName, platform::Render::TShaderPermutationSparseInt, __VA_ARGS__)
