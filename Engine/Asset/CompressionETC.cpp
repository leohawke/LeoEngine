...
namespace etc
{
	...
	static uint8 const selector_index_to_etc1[] = { 3, 2, 0, 1 };
	...
	static uint16 const color8_to_etc_block_config_0_255[2][33] =
	{
	...
	static uint16 const color8_to_etc_block_config_1_to_254[254][12] =
	{
	...
	T* IndirectRadixSort(uint32 num_indices, T* indices0, T* indices1, Q const * keys, uint32 key_ofs, uint32 key_size, bool init_indices)
	{
		lassume((key_ofs >= 0) && (key_ofs < sizeof(T)));
		lassume((key_size >= 1) && (key_size <= 4));
...
#define IRS_GET_KEY(p) (*reinterpret_cast<uint32 const *>(reinterpret_cast<uint8 const *>(keys + *(p)) + key_ofs))
#define IRS_GET_KEY_FROM_INDEX(i) (*reinterpret_cast<uint32 const *>(reinterpret_cast<uint8 const *>(keys + (i)) + key_ofs))
		...
		if (4 == key_size)
		{
			...
			for (; p != q; ++ p)
			{
				uint32 const key = IRS_GET_KEY(p);
		...
		else if (2 == key_size)
		{
			...
			for (; p != q; p += 2)
			{
				uint32 const key0 = IRS_GET_KEY(p + 0);
				uint32 const key1 = IRS_GET_KEY(p + 1);
		...
		for (uint32 pass = 0; pass < key_size; ++ pass)
		{
			uint32 const * hist_ptr = &hist[pass << 8];
			...
			uint32 const pass_shift = pass << 3;
			...
			for (; p != q; p += 2)
			{
				...
				uint32 c0 = (IRS_GET_KEY_FROM_INDEX(index0) >> pass_shift) & 0xFF;
				uint32 c1 = (IRS_GET_KEY_FROM_INDEX(index1) >> pass_shift) & 0xFF;

				if (c0 == c1)
				{
					uint32 dst_offset0 = offsets[c0];
			...
			{
				uint32 index = *p;
				uint32 c = (IRS_GET_KEY_FROM_INDEX(index) >> pass_shift) & 0xFF;

				uint32 dst_offset = offsets[c];
...
namespace etc
{
	uint8 TexCompressionETC1::quant_5_tab_[256 + 16];
	...
	uint16 TexCompressionETC1::etc1_inverse_lookup_[2 * 8 * 4][256];      // [diff/modifier_table/selector][desired_color]
	...
	TexCompressionETC1::Results::Results()
		: error_(std::numeric_limits<uint64>::max()),
	...
	TexCompressionETC1::Results& TexCompressionETC1::Results::operator=(TexCompressionETC1::Results const & rhs)
	{
		if (this != &rhs)
		{
			...
			error_ = rhs.error_;
			lassume(selectors_.size() == rhs.selectors_.size());
	...
	TexCompressionETC1::ETC1SolutionCoordinates::ETC1SolutionCoordinates(int r, int g, int b, uint32 inten_table, bool color4)
	...
	TexCompressionETC1::ETC1SolutionCoordinates::ETC1SolutionCoordinates(ARGBColor32 const & c, uint32 inten_table, bool color4)
		: unscaled_color_(c),
			inten_table_(inten_table),
			color4_(color4)
	{
	...
	void TexCompressionETC1::ETC1SolutionCoordinates::ScaledColor(uint8& br, uint8& bg, uint8& bb) const
	{
		uint8 const r = unscaled_color_.r();
		uint8 const g = unscaled_color_.g();
		uint8 const b = unscaled_color_.b();
	...
	TexCompressionETC1::PotentialSolution::PotentialSolution()
		: error_(std::numeric_limits<uint64>::max()), valid_(false)
	{
	}
	...
	TexCompressionETC1::TexCompressionETC1()
	{
		...
		if (!lut_inited_)
		{
			...
			if (!lut_inited_)
			{
				for (uint32 diff = 0; diff < 2; ++ diff)
				{
					uint32 const limit = diff ? 32 : 16;

					for (uint32 inten = 0; inten < 8; ++ inten)
					{
						for (uint32 selector = 0; selector < 4; ++ selector)
						{
							uint32 const inverse_table_index = diff + (inten << 1) + (selector << 4);
							for (uint32 color = 0; color < 256; ++ color)
							{
								uint32 best_err = std::numeric_limits<uint32>::max();
								uint32 best_packed_c = 0;
								for (uint32 packed_c = 0; packed_c < limit; ++ packed_c)
								{
									int v = ETC1DecodeValue(diff, inten, selector, packed_c);
									uint32 err = abs(v - static_cast<int>(color));
									if (err < best_err)
									{
										best_err = err;
								...
								lassume(best_err <= 255);
								etc1_inverse_lookup_[inverse_table_index][color]
									= static_cast<uint16>(best_packed_c | (best_err << 8));
							}
						}
					}
				}
				...
				{
					int v = clamp(i - 8, 0, 255);
					quant_5_tab_[i] = static_cast<uint8>(expand5[Mul8Bit(v, 31)]);
				}
			}
		}
	...
	uint64 TexCompressionETC1::EncodeETC1BlockInternal(ETC1Block& dst_block, ARGBColor32 const * argb, TexCompressionMethod method)
	{
		lassume(argb);
		...
		for (uint32 flip = 0; flip < 2; ++ flip)
		{
			for (uint32 use_color4 = 0; use_color4 < 2; ++ use_color4)
			{
				uint64 trial_err = 0;
				...
				for (subblock = 0; subblock < 2; ++ subblock)
				{
					if (flip)
					{
						memcpy(subblock_pixels, argb + subblock * 8, sizeof(uint32) * 8);
					}
					else
					{
					...
					results[2].error_ = std::numeric_limits<uint64>::max();
					...
					if (params.quality_ >= TCM_Balanced)
					{
						...
						uint32 const refinement_error_thresh0 = 3000;
						uint32 const refinement_error_thresh1 = 6000;
		...
		lassume(best_use_color4 || ((std::min({ dr, dg, db }) >= -4) && (std::min({ dr, dg, db }) <= 3)));

		if (best_use_color4)
		{
			dst_block.r = static_cast<uint8>((best_results[0].block_color_unscaled_.r() << 4) | best_results[1].block_color_unscaled_.r());
			dst_block.g = static_cast<uint8>((best_results[0].block_color_unscaled_.g() << 4) | best_results[1].block_color_unscaled_.g());
			dst_block.b = static_cast<uint8>((best_results[0].block_color_unscaled_.b() << 4) | best_results[1].block_color_unscaled_.b());
		}
		else
		{
			if (dr < 0)
			...
			dst_block.r = static_cast<uint8>((best_results[0].block_color_unscaled_.r() << 3) | dr);
			if (dg < 0)
			{
				dg += 8;
			}
			dst_block.g = static_cast<uint8>((best_results[0].block_color_unscaled_.g() << 3) | dg);
			if (db < 0)
			{
				db += 8;
			}
			dst_block.b = static_cast<uint8>((best_results[0].block_color_unscaled_.b() << 3) | db);
		}

		dst_block.cw_diff_flip = static_cast<uint8>((best_results[1].block_inten_table_ << 2)
		...
		if (best_flip)
		{
			...
			// { 0, 2 }, { 1, 2 }, { 2, 2 }, { 3, 2 },
			// { 0, 3 }, { 1, 3 }, { 2, 3 }, { 3, 3 }
			uint8 const * selectors0 = &best_results[0].selectors_[0];
			uint8 const * selectors1 = &best_results[1].selectors_[0];
			for (int x = 3; x >= 0; -- x)
			{
				uint32 b;
		...
		else
		{
			...
			{
				uint8 const * selectors = &best_results[subblock].selectors_[4];
		...
		dst_block.msb = static_cast<uint16>((selector1 >> 8) | ((selector1 & 0xFF) << 8));
		dst_block.lsb = static_cast<uint16>((selector0 >> 8) | ((selector0 & 0xFF) << 8));
	...
	uint32 TexCompressionETC1::ETC1DecodeValue(uint32 diff, uint32 inten, uint32 selector, uint32 packed_c) const
	{
#ifndef NDEBUG
		uint32 const limit = diff ? 32 : 16;
		lassume((diff < 2) && (inten < 8) && (selector < 4) && (packed_c < limit));
#endif
		int c;
		if (diff)
		{
	...
	uint64 TexCompressionETC1::PackETC1UniformBlock(ETC1Block& block, ARGBColor32 const * argb) const
	{
		lassume(etc1_inverse_lookup_[0][255]);

		static uint32 const next_comp[] = { 1, 2, 0, 1 };

		uint8 const * color = reinterpret_cast<uint8 const *>(argb);
		...
		for (uint32 i = 0; (i < 3) && (best_err != 0); ++ i)
		{
			uint32 const c1 = color[next_comp[i + 0]];
			uint32 const c2 = color[next_comp[i + 1]];

			int const delta_range = 1;
			for (int delta = -delta_range; (delta <= delta_range) && (best_err != 0); ++ delta)
			{
				int const c_plus_delta = clamp(color[i] + delta, 0, 255);
				...
				do
				{
...
#ifndef NDEBUG
					{
						uint32 diff = x & 1;
						uint32 inten = (x >> 1) & 7;
						uint32 selector = (x >> 4) & 3;
						uint32 p0 = (x >> 8) & 255;
						lassume(ETC1DecodeValue(diff, inten, selector, p0) == static_cast<uint32>(c_plus_delta));
					}
#endif

					uint32 const row = x & 0xFF;
					lassume(row < 64);
					lassume(row < 64);
					uint16 const * inverse_table = etc1_inverse_lookup_[row];
					uint16 p1 = inverse_table[c1];
					uint16 p2 = inverse_table[c2];
					uint32 const trial_err = sqr(c_plus_delta - color[i])
						+ sqr(p1 >> 8) + sqr(p2 >> 8);
					if (trial_err < best_err)
					{
						best_err = trial_err;
		...
		uint32 const diff = best_x & 1;
		uint32 const inten = (best_x >> 1) & 7;

		block.cw_diff_flip = static_cast<uint8>(((inten | (inten << 3)) << 2) | (diff << 1));

		uint32 const etc1_selector = selector_index_to_etc1[(best_x >> 4) & 3];
		memset(&block.msb, (etc1_selector & 2) ? 0xFF : 0, 2);
		memset(&block.lsb, (etc1_selector & 1) ? 0xFF : 0, 2);

		uint8* bytes = &block.r;
		uint32 const best_packed_c0 = (best_x >> 8) & 255;
		if (diff)
		{
			bytes[best_i] = static_cast<uint8>(best_packed_c0 << 3);
			bytes[next_comp[best_i + 0]] = static_cast<uint8>(best_packed_c1 << 3);
			bytes[next_comp[best_i + 1]] = static_cast<uint8>(best_packed_c2 << 3);
		}
		else
		{
			bytes[best_i] = static_cast<uint8>(best_packed_c0 | (best_packed_c0 << 4));
			bytes[next_comp[best_i + 0]] = static_cast<uint8>(best_packed_c1 | (best_packed_c1 << 4));
			bytes[next_comp[best_i + 1]] = static_cast<uint8>(best_packed_c2 | (best_packed_c2 << 4));
		}

		return best_err;
	}

	uint32 TexCompressionETC1::PackETC1UniformPartition(Results& results, uint32 num_colors, ARGBColor32 const * argb,
			bool use_diff, ARGBColor32 const * base_color5_unscaled) const
	{
		...
		for (uint32 i = 0; (i < 3) && (best_err != 0); ++ i)
		{
			...
			for (int delta = -delta_range; (delta <= delta_range) && (best_err != 0); ++ delta)
			{
				...
				do
				{
					uint32 const x = *table;
					++ table;
					uint32 const diff = x & 1;
					if (static_cast<uint32>(use_diff) != diff)
					{
						if (0xFFFF == *table)
						{
							break;
...
#ifndef NDEBUG
					{
						uint32 const inten = (x >> 1) & 7;
						uint32 const selector = (x >> 4) & 3;
						uint32 const p0 = (x >> 8) & 0xFF;
						lassume(ETC1DecodeValue(diff, inten, selector, p0) == static_cast<uint32>(c_plus_delta));
					}
#endif
		...
		if (std::numeric_limits<uint32>::max() == best_err)
		{
			return best_err;
		}
		...
		uint32 const best_packed_c0 = (best_x >> 8) & 255;
		results.block_color_unscaled_[best_i] = static_cast<uint8>(best_packed_c0);
		results.block_color_unscaled_[next_comp[best_i + 0]] = static_cast<uint8>(best_packed_c1);
		results.block_color_unscaled_[next_comp[best_i + 1]] = static_cast<uint8>(best_packed_c2);
	...
	void TexCompressionETC1::InitSolver(Params const & params, Results& result)
	{
		...
		lassume(8 == params.num_src_pixels_);
		...
		bb_ = clamp(static_cast<int>(avg_color_[2] * limit_ / 255.0f + 0.5f), 0, limit_);
	...
	{
		uint32 const n = params_->num_src_pixels_;
		...
		for (int zdi = 0; zdi < scan_delta_size; ++ zdi)
		{
			...
			for (int ydi = 0; ydi < scan_delta_size; ++ ydi)
			{
				...
				for (int xdi = 0; xdi < scan_delta_size; ++ xdi)
				{
					...
					uint32 const max_refinement_trials = (TCM_Speed == params_->quality_) ? 2 : ((0 == (xd | yd | zd)) ? 4 : 2);
					for (uint32 refinement_trial = 0; refinement_trial < max_refinement_trials; ++ refinement_trial)
					{
						uint8 const * selectors = best_solution_.selectors_;

						int delta_sum_r = 0, delta_sum_g = 0, delta_sum_b = 0;
						ARGBColor32 const base_color = best_solution_.coords_.ScaledColor();
						for (uint32 r = 0; r < n; ++ r)
						{
							uint32 const s = *selectors;
							++ selectors;
							int const yd0 = GetModifier(best_solution_.coords_.inten_table_, s);
							// Compute actual delta being applied to each pixel, taking into account clamping.
							delta_sum_r += clamp(static_cast<int>(base_color.r()) + yd0, 0, 255) - base_color.r();
							delta_sum_g += clamp(static_cast<int>(base_color.g()) + yd0, 0, 255) - base_color.g();
							delta_sum_b += clamp(static_cast<int>(base_color.b()) + yd0, 0, 255) - base_color.b();
						}
						...
						int const br1 = clamp(static_cast<int>((avg_color_[0] - avg_delta_r_f) * limit_ / 255.0f + 0.5f), 0, limit_);
						int const bg1 = clamp(static_cast<int>((avg_color_[1] - avg_delta_g_f) * limit_ / 255.0f + 0.5f), 0, limit_);
						int const bb1 = clamp(static_cast<int>((avg_color_[2] - avg_delta_b_f) * limit_ / 255.0f + 0.5f), 0, limit_);
		...
		{
			result_->error_ = std::numeric_limits<uint64>::max();
			return false;
		}
...
#ifndef NDEBUG
		{
			...
			for (uint32 i = 0; i < n; ++ i)
			{
				actual_err += sqr(src_pixels[i].r() - block_colors[selectors[i]].r())
					+ sqr(src_pixels[i].g() - block_colors[selectors[i]].g())
					+ sqr(src_pixels[i].b() - block_colors[selectors[i]].b());
			}

			lassume(actual_err == best_solution_.error_);
		}
#endif      
	...
	bool TexCompressionETC1::EvaluateSolution(ETC1SolutionCoordinates const & coords, PotentialSolution& trial_solution, PotentialSolution& best_solution)
	{
		...
		if (params_->constrain_against_base_color5_)
		{
			...
			if ((std::min({ dr, dg, db }) < -4) || (std::max({ dr, dg, db }) > 3))
			{
				return false;
			}
		}
		...
		trial_solution.error_ = std::numeric_limits<uint64>::max();

		for (uint32 inten_table = 0; inten_table < 8; ++ inten_table)
		{
			ARGBColor32 block_colors[4];
			...
			for (uint32 c = 0; c < N; ++ c)
			{
				...
				uint32 best_selector_index = 0;
				uint32 best_err = sqr(src_pixel.r() - block_colors[0].r())
					+ sqr(src_pixel.g() - block_colors[0].g())
					+ sqr(src_pixel.b() - block_colors[0].b());

				uint32 trial_err = sqr(src_pixel.r() - block_colors[1].r())
					+ sqr(src_pixel.g() - block_colors[1].g())
					+ sqr(src_pixel.b() - block_colors[1].b());
				if (trial_err < best_err)
				{
					best_err = trial_err;
				...
				trial_err = sqr(src_pixel.r() - block_colors[2].r())
					+ sqr(src_pixel.g() - block_colors[2].g())
					+ sqr(src_pixel.b() - block_colors[2].b());
				if (trial_err < best_err)
				{
					best_err = trial_err;
				...
				trial_err = sqr(src_pixel.r() - block_colors[3].r())
					+ sqr(src_pixel.g() - block_colors[3].g())
					+ sqr(src_pixel.b() - block_colors[3].b());
				if (trial_err < best_err)
				{
					best_err = trial_err;
				...
				temp_selectors_[c] = static_cast<uint8>(best_selector_index);
	...
	bool TexCompressionETC1::EvaluateSolutionFast(ETC1SolutionCoordinates const & coords, PotentialSolution& trial_solution, PotentialSolution& best_solution)
	{
		...
		for (int inten_table = 7; inten_table >= 0; -- inten_table)
		{
			...
			// 0   1   2   3
			//   01  12  23
			uint32 const block_inten_midpoints[3] = { block_inten[0] + block_inten[1],
			...
			if (sorted_luma_ptr_[N - 1] * 2 < block_inten_midpoints[0])
			{
				...
				{
					uint32 const min_err = abs(static_cast<int>(block_inten[0] - sorted_luma_ptr_[N - 1]));
				...
				for (uint32 c = 0; c < N; ++ c)
				{
					total_err += sqr(block_colors[0].r() - src_pixels[c].r())
						+ sqr(block_colors[0].g() - src_pixels[c].g())
						+ sqr(block_colors[0].b() - src_pixels[c].b());
				}
			}
			else if (sorted_luma_ptr_[0] * 2 >= block_inten_midpoints[2])
			{
				...
				{
					uint32 const min_err = abs(static_cast<int>(sorted_luma_ptr_[0] - block_inten[3]));
					if (min_err >= trial_solution.error_)
					{
						continue;
					}
				...
				for (uint32 c = 0; c < N; ++ c)
				{
					total_err += sqr(block_colors[3].r() - src_pixels[c].r())
						+ sqr(block_colors[3].g() - src_pixels[c].g())
						+ sqr(block_colors[3].b() - src_pixels[c].b());
				}
			}
			else
			{
				uint32 cur_selector = 0;
				for (uint32 c = 0; c < N; ++ c)
				{
					uint32 const y = sorted_luma_ptr_[c];
					while (y * 2 >= block_inten_midpoints[cur_selector])
					{
						...
						if (cur_selector > 2)
						{
							while (c < N)
							{
								uint32 const sorted_pixel_index = sorted_luma_indices_[c];
								temp_selectors_[sorted_pixel_index] = 3;
								total_err += sqr(block_colors[3].r() - src_pixels[sorted_pixel_index].r())
									+ sqr(block_colors[3].g() - src_pixels[sorted_pixel_index].g())
									+ sqr(block_colors[3].b() - src_pixels[sorted_pixel_index].b());
								++ c;
							}
							break;
						}
					...
					{
						uint32 const sorted_pixel_index = sorted_luma_indices_[c];
						temp_selectors_[sorted_pixel_index] = static_cast<uint8>(cur_selector);
						total_err += sqr(block_colors[cur_selector].r() - src_pixels[sorted_pixel_index].r())
							+ sqr(block_colors[cur_selector].g() - src_pixels[sorted_pixel_index].g())
							+ sqr(block_colors[cur_selector].b() - src_pixels[sorted_pixel_index].b());
					}
				}
			}
	...
	TexCompressionETC2RGB8::TexCompressionETC2RGB8()
	{
		...
		etc1_codec_ = std::make_shared<TexCompressionETC1>();
	}
	...
	TexCompressionETC2RGB8A1::TexCompressionETC2RGB8A1()
	{
		...
		etc1_codec_ = std::make_shared<TexCompressionETC1>();
		etc2_rgb8_codec_ = std::make_shared<TexCompressionETC2RGB8>();
	}
