...
namespace platform {
	...
	template<typename type>
	leo::coroutine::Task<type> Read(leo::coroutine::FileAsyncStream& stream)
	{
		type value;
	...
	public:
		...
		virtual leo::coroutine::Task<void> GetAwaiter(leo::coroutine::FileAsyncStream& file) = 0;
	...
	public:
		...
		*/
		...
		leo::coroutine::Task<void> GetAwaiter(leo::coroutine::FileAsyncStream& file) override {
			//read header
			GeomertySectionHeader header;//common header had read
			...
			leo::uint8 VertexElmentsCount = co_await Read<leo::uint8>(file);
			auto& vertex_elements = mesh_asset->GetVertexElementsRef();
			vertex_elements.reserve(VertexElmentsCount);
			for (auto i = 0; i != VertexElmentsCount; ++i) {
				Render::Vertex::Element element;
			...
			auto& sub_meshes = mesh_asset->GetSubMeshDescesRef();
			auto sub_mesh_count = co_await Read<leo::uint8>(file);
			for (auto i = 0; i != sub_mesh_count; ++i) {
				asset::MeshAsset::SubMeshDescrption mesh_desc;
				mesh_desc.MaterialIndex = co_await Read<leo::uint8>(file);
				auto lods_count = co_await Read<leo::uint8>(file);

				auto pc =co_await Read<leo::math::data_storage<float, 3>>(file);
				auto po =co_await Read<leo::math::data_storage<float, 3>>(file);
				auto tc =co_await Read<leo::math::data_storage<float, 2>>(file);
				auto to =co_await Read<leo::math::data_storage<float, 2>>(file);

				for (auto lod_index = 0; lod_index != lods_count; ++lod_index) {
					asset::MeshAsset::SubMeshDescrption::LodDescription lod_desc;
					if (index16bit) {
						lod_desc.VertexNum = co_await Read<leo::uint16>(file);
						lod_desc.VertexBase = co_await Read<leo::uint16>(file);
						lod_desc.IndexNum = co_await Read<leo::uint16>(file);
						lod_desc.IndexBase = co_await Read<leo::uint16>(file);
					}
					else {
						lod_desc.VertexNum = co_await Read<leo::uint32>(file);
						lod_desc.VertexBase = co_await Read<leo::uint32>(file);
						lod_desc.IndexNum = co_await Read<leo::uint32>(file);
						lod_desc.IndexBase = co_await Read<leo::uint32>(file);
					}
					mesh_desc.LodsDescription.emplace_back(lod_desc);
				}
				sub_meshes.emplace_back(std::move(mesh_desc));
	...
	public:
		...
		leo::coroutine::Task<std::shared_ptr<AssetType>> GetAwaiter()
		{
			...
			header.Machine = co_await Read<leo::uint16>(file);
			header.NumberOfSections = co_await Read<leo::uint16>(file);
			header.FirstSectionOffset = co_await Read<leo::uint16>(file);

			file.Skip(header.FirstSectionOffset);

			for (auto i = 0; i != header.NumberOfSections; ++i) {
				SectionCommonHeader common_header;
				common_header.SectionIndex = co_await Read<leo::uint16>(file);
				common_header.NextSectionOffset = co_await Read<leo::uint32>(file);
				co_await file.Read(common_header.Name, sizeof(common_header.Name));
				common_header.Size = co_await Read<leo::uint32>(file);
