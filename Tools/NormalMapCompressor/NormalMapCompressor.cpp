...
#include <Engine/Asset/CompressionBC.hpp>
...
namespace
{
	...
	void CompressNormalMapSubresource(uint32_t width, uint32_t height, std::vector<Color>& in_data,
		EFormat new_format, ElementInitData& new_data, std::vector<uint8_t>& new_data_block)
	{
	...
	void DecompressNormalMapSubresource(uint32_t width, uint32_t height, EFormat restored_format,
		ElementInitData& restored_data, std::vector<uint8_t>& restored_data_block, EFormat com_format, ElementInitData const& com_data)
	{
		lunused(restored_format);

		std::unique_ptr<TexCompression> tex_codec;
		switch (com_format)
		{
		case EF_BC3:
			tex_codec = make_unique<TexCompressionBC3>();
			break;

		case EF_BC5:
			tex_codec = make_unique<TexCompressionBC5>();
			break;

		default:
			throw "Compression formats other than BC3 and BC5 are not supported";
		}
		...
		if (restored_format != EF_ARGB8)
		{
			...
			platform::X::ResizeTexture(&argb8_normals[0], width * 4, width * height * 4, EF_ARGB8, width, height, 1,
	...
	float MSESubresource(uint32_t width, uint32_t height, EFormat in_format,
		ElementInitData const& in_data, EFormat new_format, ElementInitData const& new_data)
	{
		...
		if ((out_width != width) || (out_height != height))
		{
			...
			platform::X::ResizeTexture(&resized_restored_data_block[0],
	...
	void CompressNormalMap(std::string const& in_file, std::string const& out_file, EFormat new_format)
	{
		platform::X::path path = in_file;
		...
		for (size_t sub_res = 0; sub_res < in_array_size; ++sub_res)
		{
			...
			for (uint32_t mip = 0; mip < in_num_mipmaps; ++mip)
			{
				...
				platform::X::ResizeTexture(&in_color[sub_res * in_num_mipmaps + mip][0],
